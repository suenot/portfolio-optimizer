...
function MMLogoSVG({ size = 28 }: { size?: number }) {
  return (
    <svg
...
export default function Navbar() {
  ...
  return (
    <nav
      ...
      style={{
        background: scrolled
          ? 'hsla(240, 10%, 3.9%, 0.85)'
          : 'transparent',
        backdropFilter: scrolled ? 'blur(12px)' : 'none',
        borderBottom: scrolled
          ? '1px solid hsl(240 3.7% 20%)'
    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <div className="flex flex-col">
            <span
              ...
              style={{ color: 'var(--accent-dark)', letterSpacing: '0.15em' }}
            >
              Portfolio Optimizer
            </span>
            <span
        ...
        <Link
          href="/optimizer"
          className="relative inline-flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold transition-all duration-200 hover:scale-105"
          style={{
            background: 'hsl(245 72% 60%)',
            color: '#fff',
            boxShadow: '0 0 20px hsla(245, 72%, 60%, 0.3)',
          }}
        >
          Open App
        </Link>
